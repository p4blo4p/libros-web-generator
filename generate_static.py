...
def get_translated_url_segment_for_generator(
    ...
    # ... (código sin cambios, removed unused logger_to_use and log variable) ...
        ...
        with worker_app_instance.test_client() as client:
            for lang in LANGUAGES:
                ...
                output_path_obj = (
                    ...
                    author_s / title_s / identifier / "index.html"  # Path es global
                )
                output_path_str = str(output_path_obj)

                if FORCE_REGENERATE_ALL or should_regenerate_page(
                    output_path_str, current_book_content_signature, manifest_data_global, log_target  # should_regenerate_page es global
                ):
...
def main():  # C901
    ...
    if not books_for_generation_full_list:
        main_process_logger.critical(
            "No hay datos de libros (app_instance.books_data está vacío o no es una lista). Saliendo."
        )
        return

    main_process_logger.info(
        f"Idiomas a procesar: {LANGUAGES_TO_PROCESS}. "
        f"{len(books_for_generation_full_list)} libros en total en los datos fuente."
    )
    ...
    if perform_full_cleanup and not args.language:
        if Path(OUTPUT_DIR).exists():  # Path, OUTPUT_DIR
            main_process_logger.info(
                f"Eliminando {OUTPUT_DIR} (generación completa o forzada sin idioma específico)"  # OUTPUT_DIR
            )
        ...
        if static_folder_path.exists() and static_folder_path.is_dir():
            ...
            if static_output_dir.exists():
                shutil.rmtree(static_output_dir)  # shutil
        ...
        if public_folder_path.exists() and public_folder_path.is_dir():
            ...
            for item in public_folder_path.iterdir():
                if item.is_file():
                    try:
                        shutil.copy2(item, public_output_dir / item.name)  # shutil
        ...
        main_process_logger.info(
            f"Asegurando que {OUTPUT_DIR} (y subdirectorios de idioma si aplica) existen."  # OUTPUT_DIR
        )
    ...
    with app_instance_main.app_context():
        with app_instance_main.test_client() as client_main:
            if not args.language or args.force_regenerate:
                _save_page_local(
                    client_main, "/", Path(OUTPUT_DIR) / "index.html", main_process_logger  # _save_page_local, Path, OUTPUT_DIR
                )
                if app_instance_main.url_map.is_endpoint_expecting('main.test_page'):
                    _save_page_local(
                        client_main, "/test/",
                        Path(OUTPUT_DIR) / "test_sitemap" / "index.html", main_process_logger  # _save_page_local, Path, OUTPUT_DIR
                    )

            for lang in LANGUAGES_TO_PROCESS:
                _save_page_local(
                    client_main, f"/{lang}/", Path(OUTPUT_DIR) / lang / "index.html", main_process_logger  # _save_page_local, Path, OUTPUT_DIR
                )
    ...
    with Pool(processes=num_processes, initializer=worker_init) as pool:  # Pool
        ...
        book_detail_task_with_args = partial(
            generate_book_detail_pages_task, config_params_manifest_tuple=task_args_tuple  # partial
        )
        results_books_list_of_lists = pool.map(book_detail_task_with_args, books_for_generation_full_list)
        for res_list in results_books_list_of_lists:
            new_manifest_entries_from_workers.extend(res_list)
        main_process_logger.info(
            f"Proceso de detalle de libros completado. {len(new_manifest_entries_from_workers)} "
        ...
        author_task_with_args = partial(
            generate_author_pages_task, config_params_manifest_tuple=task_args_tuple  # partial
        )
        results_authors_list_of_lists = pool.map(author_task_with_args, list(unique_author_slugs_orig))
        for res_list in results_authors_list_of_lists:
            new_manifest_entries_from_workers.extend(res_list)
        main_process_logger.info(
            f"Proceso de páginas de autor completado. "
            f"{len(new_manifest_entries_from_workers) - current_generated_count} "
        ...
        unique_book_bases_slugs = {
            ...
            for b in books_for_generation_full_list
            if b.get('author_slug') and b.get('base_title_slug')
        }
        versions_task_with_args = partial(
            generate_versions_pages_task, config_params_manifest_tuple=task_args_tuple  # partial
        )
        results_versions_list_of_lists = pool.map(versions_task_with_args, list(unique_book_bases_slugs))
        for res_list in results_versions_list_of_lists:
            new_manifest_entries_from_workers.extend(res_list)
        main_process_logger.info(
            f"Proceso de páginas de versiones completado. "
            f"{len(new_manifest_entries_from_workers) - current_generated_count} "
    ...
    if not args.language or args.force_regenerate:
        ...
        with app_instance_main.app_context():
            with app_instance_main.test_client() as client_main:
                _save_page_local(
                    client_main, "/sitemap.xml", Path(OUTPUT_DIR) / "sitemap.xml", main_process_logger  # _save_page_local, Path, OUTPUT_DIR
                )

        save_manifest(manifest_data)  # save_manifest
    else:
        main_process_logger.info(
            f"Ejecución para idioma '{args.language}'. El manifest global no se guardó desde este script. "
            f"{len(new_manifest_entries_from_workers)} páginas del idioma (re)generadas."
        )

    main_process_logger.info(
        f"Sitio estático (o parte para el idioma '{args.language if args.language else 'todos'}') "
        f"generado en: {OUTPUT_DIR}"  # OUTPUT_DIR
    )
