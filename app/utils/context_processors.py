...
def inject_global_template_variables():
    ...
    if not current_lang or current_lang not in supported_langs:
        ...
        # Esto es un fallback, idealmente 'lang_code' siempre estará en view_args para las rutas internacionalizadas.
        current_lang = default_lang
        # current_app.logger.debug(f"Context Processor: lang_code not in view_args or unsupported. Using default: {current_lang}")
    ...
    return dict(
        ...
        # Usaremos la variable 'lang' que ya pasas a tus plantillas para determinar el idioma actual.
        # Si 'lang' no está disponible en algún template donde uses el selector, este current_lang puede servir.
        current_lang_from_context=current_lang,
        
        # Información para el selector de idioma, para construir URLs a la misma página en otro idioma
