...
def create_app(config_class=Config):
    ...
    if not app.debug or app.config.get('FORCE_DETAILED_LOGGING', False):
        ...
        is_root_logger = not app.logger.name or app.logger.name == app.name
        ...
        if not app.logger.handlers or \
           (len(app.logger.handlers) == 1 and isinstance(app.logger.handlers[0], logging.StreamHandler) and \
